...
def get_box_annotator(type=None, color=sv.ColorPalette.DEFAULT):
    return sv.RoundBoxAnnotator(color=color)
...
class ServicesDrawer:
    ...
    def init_draw(self, service_name, metadata):
        ...
        drawer.box_annotator_violation = get_box_annotator(color=sv.Color.from_hex("#ff0000"))
        for polygons in metadata["polygons"]:
            if len(polygons["zone"]) == 4:
                zone = np.array(polygons["zone"]).astype(int)
                zone_center = PolygonZoneCustom(
                    name=polygons["name"],
                    polygon=zone,
                    triggering_anchors=[sv.Position.CENTER],
                )
                zone_bottom_center = PolygonZoneCustom(
                    name=polygons["name"],
                    polygon=zone,
                    triggering_anchors=[sv.Position.BOTTOM_CENTER],
                )

                zone_rectangle = PolygonZoneCustom(
                    name=polygons["name"],
                    polygon=zone,
                    triggering_anchors=[
        ...
        for lines in metadata["lines"]:
            if lines and len(lines["start"]) == 2 and len(lines["end"]) == 2:
                line_start = sv.Point(*lines["start"])
                line_end = sv.Point(*lines["end"])
                line = LineZoneCustom(
                    name=lines["name"],
                    start=line_start,
                    end=line_end,
...
class PolygonZoneCustom(sv.PolygonZone):
    def __init__(
        self, name: str, polygon: np.ndarray, triggering_anchors: List[sv.Position]
    ):
            ...
            for detection_idx, center_coordinates in enumerate(anchors_coordinates):
                ...
                for line, height in zip(lines, line_heights):
                    cv2.putText(
                        ...
                        text=line,
                        org=(
                            text_background_xyxy[0] + self.text_padding,
                            current_y + height,
                        ),
                        fontFace=font,
                        fontScale=self.text_scale,
