...
async def update_camera(id: str, data: dict):
    try:
        ...
        old_services = camera.get("services", {})
        if "services" in data:
            for _service in data["services"]:
                ...
                old_services[service_name]["polygons"] = polygons_data
        Camera.update_one(
            {"camera_id": id},
            {
                "$set": {
...
async def add_camera(
    ...
    try:
        ...
        if existing_camera is not None:
            raise HTTPException(
                status_code=400, detail=f"Camera with id {camera_id} already exists."
            )
...
async def update_camera_status(id: str, status: bool):
    try:
        ...
        Camera.update_one({"camera_id": id}, {"$set": {"status": status}})
